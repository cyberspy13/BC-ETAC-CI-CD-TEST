pageextension 50202 ETACItemCardPage extends "Item Card"
{
    trigger OnOpenPage()
    begin
        ...
        if Grec_ETACControlMngm.FindFirst() then begin
            repeat
                if Grec_ETACControlMngm."Select All" then begin
                    Gcdu_ETACPermissionCodeUnit.ETACAllItemssBlock(UserId);
        ...
        Grec_ETACControlMngm.SetRange("Source Table", Grec_ETACControlMngm."Source Table"::Item);
        if Grec_ETACControlMngm.FindFirst() then
            if Grec_ETACControlMngm."View Record" = true then
                Error('Current user cannot open the Item Card page. Please review Control Management Page');
